...
def get_box_coordinates(
    img, model, device, showOriginalImage=False, showPoints=False, showPlotMaskWithHighestScore=True
):
    """
    Parameters
    ----------
...
def get_image_with_box_corners(frame, points_dict):
    ...
    colors_dict = {"blue": (0, 0, 255), "red": (255, 0, 0), "green": (0, 255, 0), "yellow": (255, 255, 0)}
    ...
    # top_right : blue
    cv2.circle(
        frame, (points_dict["top_right"][1], points_dict["top_right"][0]), circle_radius, colors_dict["blue"], -1
    )

    # bottom_right : green
    cv2.circle(
        frame, (points_dict["bottom_right"][1], points_dict["bottom_left"][0]), circle_radius, colors_dict["green"], -1
    )

    # bottom_left : yellow
    cv2.circle(
        frame, (points_dict["bottom_left"][1], points_dict["bottom_left"][0]), circle_radius, colors_dict["yellow"], -1
    )

    return frame


def annotate_frame_with_mask(
    img, model, device, showOriginalImage=False, showPoints=False, showPlotMaskWithHighestScore=True
):
    """
    Parameters
    ...
    annotated_img : np.array
        Annotated image with the mask as an overlay.
    """
    ...
    annotated_img[np.where(img_mask == 1)] = [0, 255, 0]  # Overlay the mask in green color
